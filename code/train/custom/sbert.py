...
from custom.transformer import Transformer
...
class SentenceTransformer(nn.Sequential):
    ...
    :param from_scratch: Whether to train model from scratch.
    ...
    def __init__(self, model_name_or_path: str = None, modules: Iterable[nn.Module] = None, device: str = None,
                 from_scratch: bool = False, max_length: int = 128):
        if model_name_or_path is not None and model_name_or_path != "":
                ...
                if not os.path.exists(model_path) or not os.listdir(model_path):
                    ...
                    try:
                        ...
                        if e.response.status_code == 404:
                            ...
                            model_path = None
                            transformer_model = Transformer(model_name_or_path, max_seq_length=max_length)
            ...
            if model_path is not None:
                if from_scratch:
                    transformer_model = Transformer(model_name_or_path, max_seq_length=max_length, from_scratch=from_scratch)
                    pooling_model = Pooling(transformer_model.get_word_embedding_dimension())
                    modules = [transformer_model, pooling_model]
                else:
    ...
    def save(self, path):
        ...
        with open(os.path.join(path, 'modules.json'), 'w') as fOut:
            contained_modules[0]["type"] = "sentence_transformers.models.Transformer"
